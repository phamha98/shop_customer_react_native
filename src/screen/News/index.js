...
export default function index({navigation}) {
  const {token, datanew, setDatanew} = useContext(AppContext);
  const [load, setLoad] = useState(false);
  const [refreshing, setRefreshing] = useState(false);
  const [progess, setProgess] = useState(true);
  //**** new */
  useEffect(() => {
    fetch(localhost+'api/newproductdetails', {
      method: 'GET',
      headers: {
        token: token,
      },
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.container}>
        ...
        onClickLeft={()=>navigation.openDrawer()}
          ...
          title="   SHOP"
        />
        <Search background="#CEC5BE" />

        <View style={[styles.product, {marginBottom: 150}]}>
          <Text style={{fontWeight: 'bold', fontSize: 20}}>{'  '}New</Text>
